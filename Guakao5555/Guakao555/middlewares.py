...
class Guakao555SpiderMiddleware(object):
...
class Guakao555DownloaderMiddleware(object):
    # Not all methods need to be defined. If a method is not defined,
...
class ProxyMiddleware(object):
    ...
    def process_response(self, request, response, spider):
        ...
        if response.status != 200:
            # 改变ip为不可用
            ip = request.meta['proxy'].split('http://')[1]
            self.col.find_one_and_update({"_id": ip['_id']}, {"$set": {"flag": "0"}})
